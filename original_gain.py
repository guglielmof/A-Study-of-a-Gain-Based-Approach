...
for rl in recall_levels:
    ...
    results[(results['tau']<0) & (results['pval']!='null')]['pval'] = 'not significant'
    ...
    #    plt.text(row['x1'], row['tau'], row['topic'], fontsize=
    axes[1].set_xlabel("topics", fontsize=20)
    axes[1].set_ylabel(r"Kendall's $\tau$", fontsize=20)
    handles, labels = axes[1].get_legend_handles_labels()
    axes[1].get_legend().remove()
    plt.legend(handles, labels, ncol=len(labels), fontsize=14, loc='upper center',
